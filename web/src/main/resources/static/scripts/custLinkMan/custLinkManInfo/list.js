layui.use(['form', 'layer', 'table', 'laytpl', 'laydate'], function () {
    ...
    var tableIns = table.render({
        elem: '#List',
        url: web.rootPath() + 'custLinkManInfo/list',
        cellMinWidth: 95,
        page: true,
        ...
        cols: [[
            {type: "checkbox", fixed: "left", width: 50},
                    ...
                    {field: 'custLinkName', title: '企业名称', minWidth: 100, align: "center"},
                    {field: 'linkman', title: '联系人名字', minWidth: 100, align: "center"},
                    {
                        field: 'sex', title: '性别', width: 80, align: "center", templet: function (d) {
                            if (d.sex == '1') {
                                return "<button class=\"layui-btn layui-btn-normal layui-btn-xs\">男</button>";
                            } else if (d.sex == '0') {
                                return "<button class=\"layui-btn layui-btn-danger layui-btn-xs\">女</button>";
                            }
                        }
                    },
    ...
    table.on('toolbar(List-toolbar)', function (obj) {
        ...
        if (obj.event == 'add') {
            layer.open({
                ...
                fixed: false,
                maxmin: true,
                content: web.rootPath() + 'custLinkManInfo/add.html'
            });
        };
        ...
        if(obj.event == 'export'){
            ...
            //获取搜索条件值
            var parameterName = $("#searchForm").find("input[name='parameterName']").val().trim();
            var url = web.rootPath()+"custLinkManInfo/export?parameterName="+parameterName;
            $.fileDownload(url, {
                httpMethod: 'POST',
        ...
        switch (obj.event) {
            case 'update':
                layer.open({
                    ...
                    fixed: false,
                    maxmin: true,
                    content: web.rootPath() + "custLinkManInfo/" + data.id + ".html?_"
                });
                break;
            case 'delete':
                var index = layer.confirm('确定要删除?', {
                    ...
                    $.ajax({
                        url: web.rootPath() + "custLinkManInfo/delete/" + data.id,
                        type: "delete",
